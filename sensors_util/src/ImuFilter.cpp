...
ImuFilter::ImuFilter():
  ...
  zeta_(0.0), gain_(0.0), is_initialized_(false) {}
...
void ImuFilter::getInitialOrientation(
    float gx, float gy, float gz, 
    float ax, float ay, float az, 
    float mx, float my, float mz) {
  double roll = atan2(ay, az);
  double pitch = -atan2(ax, sqrt(ay * ay + az * az));
  double hx = mx * cos(pitch) + my * pitch * sin(roll) + mz * cos(roll) * sin(pitch);
  double hy = my * cos(roll) - mz * sin(roll);
...
void ImuFilter::madgwickAHRSupdate(
  ...
  float dt) {
  if (!is_initialized_) {
    getInitialOrientation(gx, gy, gz, ax, ay, az, mx, my, mz);
    w_bx_ = 0.0;
    w_by_ = 0.0;
    w_bz_ = 0.0;
  ...
  if (!std::isfinite(mx) || !std::isfinite(my) || !std::isfinite(mz)) {
	...
	if (!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
		...
		recipNorm = 1.f / sqrt(s0 * s0 + s1 * s1 + s2 * s2 + s3 * s3); // normalize step magnitude
		s0 *= recipNorm;
		s1 *= recipNorm;
		s2 *= recipNorm;
		s3 *= recipNorm;
